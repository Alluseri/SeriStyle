...
// Move #owner
$(SelBottomRow).prepend($(SelOwner)); // Breaks if too early, fix with a secondary loader script
...
var ActionBarEventListeners = [
	async (Inserted, Removed, Remained) => {
		...
		if (AddToPlaylist) {
			console.log("[SeriStyle|DEBUG] Found ATP, modding into iATP.");
			CreateClip?.remove();
			if (Modded) {
			...
			console.log("[SeriStyle|DEBUG] Found CC, modding into mCC.");
			...
			console.log("[SeriStyle|DEBUG] Found outdated mCC, repatching.");
			var InteractBtn = await FindATPInteract(); // jshint ignore: line
			if (!InteractBtn) {
				Modded.querySelector("yt-icon").innerHTML = SvgCreateClip;
				Modded.classList.remove("seristyle-ax-mcc");
				return console.log("[SeriStyle|ERROR] Can't run Mod-ATP-RepatchCC: No interact found.");
			}
...
if (Settings_DisableAutoplayScroll)
	setInterval(Settings_AlternativeDisableAutoplayScroll ? () => {
		var AutonavLabel = $(".ytp-autonav-endscreen-upnext-header");
		if (AutonavLabel) {
			var SeriLabel = $("#seristyle_autoplay") || DomUtils.BuildElement("span", { id: "seristyle_autoplay", innerText: Localize_AutoplaySuspended }, [], Element => {
		...
		var CancelBtn = $("#movie_player > div.ytp-autonav-endscreen-countdown-overlay > div > div.ytp-autonav-endscreen-button-container > button");
